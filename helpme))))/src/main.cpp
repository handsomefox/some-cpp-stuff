...
DWORD GetBaseCpuFrequency() {
	DWORD value, buffer[512];
	RegGetValueA(HKEY_LOCAL_MACHINE, R"(HARDWARE\DESCRIPTION\System\CentralProcessor\0)", "~MHz", RRF_RT_REG_DWORD, 0, &value, buffer);
	return value;
}
...
void Max32BitTime() {
	...
	ft.dwLowDateTime = static_cast<DWORD>(time); ft.dwHighDateTime = time >> 32;
...
void ArrayRdtscBenchmark() {
	...
	std::cout << std::setfill(' ') << "__rdtsc() array addition " << std::setw(17) << n << " times took " << std::setw(11) << end - start << " ticks" << "(" << (end - start) / cpufreq << " us)\n";
}
...
void Task5() {
	...
	while (matrixSize <= 2048) {
		...
		std::cout << "Matrix multiplication with objects took: " << MatrixBenchmark(object1, object2, matrixSize) << " sec\n";
		std::cout << "Matrix multiplication without objects took: " << MatrixBenchmark(matrix1, matrix2, matrixSize) << " sec\n";
		matrixSize *= 2;
	}
}
int main() {
