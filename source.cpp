...
DWORD GetBaseCpuFrequency() {
    DWORD value;
    DWORD buffer[512];

    RegGetValueA(HKEY_LOCAL_MACHINE,
        R"(HARDWARE\DESCRIPTION\System\CentralProcessor\0)", "~MHz",
        RRF_RT_REG_DWORD, nullptr, &value, buffer);

    return value;
}
DWORD cpufreq = GetBaseCpuFrequency();
...
void ArrayRdtscBenchmark() {
    ...
    std::cout << std::setfill(' ') << "__rdtsc() array addition " << std::setw(17)
        << n << " times took " << std::setw(11) << end - start << " ticks"
...
void ArrayQPCBenchmark() {
    ...
    std::cout << std::setfill(' ') << "GetPerformanceCounter() array addition "
        << n << " times took " << std::setw(11) << elapsed.QuadPart
        << " us\n"
        << std::endl;
}
...
void Task4() {
    ...
    ChangeConsoleColor(14);
    std::cout << "\nDifference in T(200000/100000) = " << ttime2 * tops2
        << std::endl;
    std::cout << "Difference in T(300000/100000) = " << ttime3 * tops3
        << std::endl;
}
void Task5() {
    ...
    std::cout << "--------------------------------\n";
    for (int i = 0; i < 3; ++i) {
        // TYPE is defined in pch.h
        ChangeConsoleColor(14);
        std::cout << "\nSize = " << matrixSize << std::endl;
        Matrix<TYPE> mObj1(matrixSize);
        Matrix<TYPE> mObj2(matrixSize);
        mObj1.generate();
        mObj2.generate();
        TYPE** mRaw1 = CreateFilledMatrix(matrixSize);
        TYPE** mRaw2 = CreateFilledMatrix(matrixSize);
        auto resObj = MatrixBenchmark(mObj1, mObj2);
        std::cout << "Matrix multiplication with objects took: " << resObj
            << " sec\n";
        auto resRaw = MatrixBenchmark(mRaw1, mRaw2, matrixSize);
        std::cout << "Matrix multiplication without objects took: " << resRaw
            << " sec\n";
        matrixSize *= 2;
    }
}
